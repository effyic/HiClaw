...
| `overrides_global_rule_id` | int    | null                                |
| `description`              | string | 描述                                  |
...
按 `hits` 降序。`type_name` 为关联敏感类型名称（类型已被物理删除时为 `null`）。`last_session_id` 是该规则最近一次命中的会话 ID（可能为 null），可作为跳转会话详情的示例入口。接口只返回 `rule_id`，规则 pattern 等展示信息需前端结合规则接口自行关联。
...
`type_name` 为关联敏感类型名称（类型已被物理删除时为 `null`）。
...
6. **统计与明细联动**：`by-rule` / `by-type` 已返回 `type_name`；规则 pattern 仍需用规则接口补充名称。`orphaned` 或已删除的规则 ID 可能查不到详情，需容错展示（如"规则 #101（已删除）"）。
