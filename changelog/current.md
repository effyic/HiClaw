...
- feat(sensitive-content/openagno): scope enabled sensitive-content rules to explicit `agno_agent.id` bindings — add Agent binding CRUD and per-Agent policy versions, Agent-specific snapshots/cache isolation, global-override following, automatic unbinding on rule deletion, and `agent_id` hit-event filtering; existing Agents intentionally start with no bindings (commit hash TBD — fill in on commit).
