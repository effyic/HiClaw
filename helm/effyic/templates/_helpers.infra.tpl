...
{{- $managerEnv := dict
  "HICLAW_NACOS_REGISTRY_URI" (include "hiclaw.nacos.registryURI" .)
  "HICLAW_NACOS_USERNAME" (.Values.nacos.auth.username | default "nacos")
  "HICLAW_NACOS_PASSWORD" (.Values.nacos.auth.password | default "nacos")
-}}
{{- if .Values.manager.env }}
{{- $managerEnv = merge $managerEnv .Values.manager.env }}
{{- end }}
{{- $_ := set $spec "env" $managerEnv }}
{{- else if .Values.manager.env }}
{{- $_ := set $spec "env" .Values.manager.env }}
{{- end }}
{{- $spec | toJson -}}
{{- end }}
